...
public class Drone3_ : MonoBehaviour
{
  public GameObject obj3; //担当の救助犬
  public Vector3 pos;
  public Vector3 pos2;
  public Vector3 pos3;
  ...
  public int exchange = 0;
    ...
    void Update()
    {
      ...
      //shpheの現在位置
      pos = obj3.transform.position;
      drone_time += Time.deltaTime;
      //捜索待機中
      if(dmode == 0 || dmode == 1 || dmode == 2 || dmode == 3){
        time += Time.deltaTime;
        if(drone2.move.hit == 5 || drone1.move.hit == 5){
          time -= Time.deltaTime;
        ...
        if(finding == 0){
          ...
          if(time <= 30){
            ...
            if(dmode == 0){
              if(drone2.count == 1 && drone1.count == 1){
                if(drone2.time >= drone1.time){
                  if((drone2.move.mode == 0) || (drone2.move.mode == 3)){
                    // drone5.dmode = 9;
                    dmode = 2;
                  }
                }
                else if (drone2.time < drone1.time){
      ...
      //捜索中の挙動
        ...
        if(seconds <= t){
          ...
          if(time <= m){
            ...
            if(move.hit == 0){
              ...
              if(j == 0){
                ...
                mmode = move.mode;
                pos3 = obj3.transform.position;
                pos4 = obj3.transform.rotation;
              }
            }
            if((move.hit >= 1 && move.hit <= 4) && (drone2.move.hit == 5 || drone1.move.hit == 5)){
              seconds -= Time.deltaTime;
              if(flag == 1){
                direction (pos3);
                //現在位置によって場合分け
            ...
            if(move.hit == 5){
              ...
              if(drone2.finding == 2 || drone1.finding == 2){
                if(flag == 1){
                  direction (pos3);
                  //現在位置によって場合分け
                  station (pos3);
          ...
          //活動時間外
            ...
            //交代要請受理
            if(drone2.dmode == 3 || drone1.dmode == 3){
              if(drone2.move.mode != 2 && drone1.move.mode != 2){
                if(drone1.exchange == 3 || drone2.exchange == 3){
                  if(move.mode == 0){
                    dmode = 5;
                    pos4 = obj3.transform.rotation;
                  }
    ...
    void searchmove (){
      if((waitplace < drone2.searchplace) && (waitplace < drone1.searchplace)){
    ...
    void direction (Vector3 die){
      ...
      obj3.transform.Rotate(new Vector3(0,-rad,0));
      move.mode = 2;
    }
    void renew2 (){
      ...
      obj3.transform.rotation = drone2.pos4;
      move.mode = 0;
      dmode = 4;
      flag = 1;
      count = 0;
    ...
    void renew1 (){
      ...
      obj3.transform.rotation = drone1.pos4;
      move.mode = 0;
      dmode = 4;
      flag = 1;
      count = 0;
    ...
    void reset2(){
      ...
      k = 0;
      i = 0;
      if(drone2.waitplace <= drone1.waitplace){
        waitplace = drone2.waitplace;
      }
      else if(drone2.waitplace > drone1.waitplace){
