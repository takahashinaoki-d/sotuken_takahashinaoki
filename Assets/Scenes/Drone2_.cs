...
public class Drone2_ : MonoBehaviour
{
  public GameObject obj2; //担当の救助犬
  public Vector3 pos;
  public Vector3 pos2;
  public Vector3 pos3;
  ...
  //次の捜索スタート場所
  public Vector3 end = new Vector3(10.0f,0.0f,15.0f);
  //捜索場所移動などの制御変数
  public  int flag = 1;
  //動ける範囲の制御
  public int left;
  ...
  public int exchange = 0;
    ...
    void Update()
    {
      ...
      if(dmode == 0 || dmode == 1 || dmode == 2 || dmode == 3){
        time += Time.deltaTime;
        if(drone1.move.hit == 5 || drone3.move.hit == 5){
          time -= Time.deltaTime;
          if (finding == 1 && drone4.dmode != 9 && drone5.dmode != 9){
            if(flag == 1){
              pos3 = obj2.transform.position;
              if(drone1.move.hit == 5){
                pos2 = drone1.obj1.transform.position;
              }
        ...
        if(finding == 0){
          ...
          if(time <= 30){
            ...
            if(dmode == 0){
              if(drone1.count == 1 && drone3.count == 1){
                if(drone1.time >= drone3.time){
                  if((drone1.move.mode == 0) || (drone1.move.mode == 3)){
                    // drone5.dmode = 9;
                    dmode = 1;
                  }
                }
                else if (drone1.time < drone3.time){
                ...
                if((drone1.move.mode == 0) || (drone1.move.mode == 3)){
                  if(drone5.dmode != 9){
                    if(flag == 1){
                      //Sphre（２）の現在位置
                      pos2 = drone1.obj1.transform.position;
                      pos4 = drone1.obj1.transform.rotation;
                      direction (pos2);
                      move.mode = 3;
                      exchange = 1;
                      //pos2の場所によって場合分け
      ...
      //捜索中の挙動
        ...
        if(seconds <= t){
          ...
          if(time <= m){
            ...
            if(move.hit == 0){
              ...
              if(j == 0){
                ...
                mmode = move.mode;
                pos3 = obj2.transform.position;
                pos4 = obj2.transform.rotation;
              }
            }
            if((move.hit >= 1 && move.hit <= 4) && (drone1.move.hit == 5 || drone3.move.hit == 5)){
              seconds -= Time.deltaTime;
              if(flag == 1){
                direction (pos3);
                //現在位置によって場合分け
            ...
            if(move.hit == 5){
              ...
              if(drone1.finding == 2 || drone3.finding == 2){
                if(flag == 1){
                  direction (pos3);
                  //現在位置によって場合分け
                  station (pos3);
          ...
          //活動時間外
            ...
            //交代要請受理
            if(drone1.dmode == 2 || drone3.dmode == 2){
              if(drone1.move.mode != 2 && drone3.move.mode != 2){
                if(drone1.exchange == 2 || drone3.exchange == 2){
                  if(move.mode == 0){
                    dmode = 5;
                    pos4 = obj2.transform.rotation;
                  }
    ...
    void searchmove (){
      if((waitplace < drone1.searchplace) && (waitplace < drone3.searchplace)){
    ...
    void direction (Vector3 die){
      ...
      obj2.transform.Rotate(new Vector3(0,-rad,0));
      move.mode = 2;
    }
    void renew1 (){
      searchplace = drone1.searchplace;
      left = drone1.left;
      right = drone1.right;
      top = drone1.top;
      bottom = drone1.bottom;
      center = drone1.center;
      end = drone1.end;
      obj2.transform.rotation = drone1.pos4;
      move.mode = 0;
      dmode = 4;
      flag = 1;
      count = 0;
      seconds = drone1.seconds;
      time = 0;
      exchange = 0;
      i = 0;
    }
    void renew3 (){
      ...
      obj2.transform.rotation = drone3.pos4;
      move.mode = 0;
      dmode = 4;
      flag = 1;
      count = 0;
    ...
    void reset2(){
      ...
      k = 0;
      i = 0;
      if(drone1.waitplace <= drone3.waitplace){
        waitplace = drone1.waitplace;
      }
      else if(drone1.waitplace > drone3.waitplace){
